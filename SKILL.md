...
description: "阿里云百炼 AI 生图助手，支持 wan2.7、qwen-image-2.0、z-image-turbo 三个模型。智能选择最佳模型、自动优化提示词、支持文生图、图生图、多图融合。当用户需要生成图片、编辑图片、设计海报、制作信息图时使用此 Skill。"
---
...
| 触发条件 | 推荐模型 | 理由 |
|---------|---------|------|
| "文字"/"排版"/"信息图"/"中文" | **qwen-image-2.0** | 文字渲染最准确 |
| "组图"/"连环画"/"故事系列" | **wan2.7** | 支持顺序生成，风格一致 |
| "快速"/"电影感"/"胶片风" | **z-image-turbo** | 速度最快，电影感强 |
| 需要图生图/图片编辑 | **wan2.7** 或 **qwen-image-2.0** | z-image-turbo 不支持 |
| 需要多图融合 | **qwen-image-2.0** | 唯一支持 |
| 无特殊要求 | **qwen-image-2.0** | 综合能力最强 |

**排除规则**：
- 图生图/图片编辑任务 → 排除 z-image-turbo
- 多图融合任务 → 只能用 qwen-image-2.0

### 3. 提示词处理（两阶段）

#### 阶段一：诊断与增强

使用 Qwen3.7 Max 评估提示词完整度（0-100）：
...
| **qwen-image-2.0** | 结构化中文，分要素描述 | "画面主体：橘猫；姿态：趴在窗台；场景：阳光充足；光线：温暖午后；风格：治愈系" |
...
# 准备图片输入
image_data = prepare_image_input("path/to/image.jpg", target_model="qwen-image-2.0")
