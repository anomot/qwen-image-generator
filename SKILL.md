...
```bash
# wan2.7
python scripts/wan_image_gen.py text2img "提示词" --model [pro/std] --size [size_value] --n [数量] --filename-prefix "[命名前缀]"

# qwen-image-2.0
python scripts/qwen_image_gen.py text2img "提示词" --model [default/pro] --size [size_value] --n [数量] --filename-prefix "[命名前缀]"

# z-image-turbo
python scripts/z_image_gen.py text2img "提示词" --size [size_value] --n [数量] --filename-prefix "[命名前缀]"
```
...
python scripts/qwen_image_gen.py text2img   "画面主体：春日花园；场景：花朵盛开，阳光明媚；光线：温暖的自然光；风格：清新治愈"   --model default --size portrait --n 1 --filename-prefix "春日海报_花朵阳光_清新治愈"

# 示例2：用户要求"用 wan2.7 生成一组小猫日常的连环画"
# → 使用 wan2.7，组图用 n=3，启用顺序生成
python scripts/wan_image_gen.py text2img   "一只小橘猫的日常：早上起床、中午玩耍、晚上睡觉"   --model pro --size landscape --n 3 --enable-sequential --filename-prefix "小猫日常_连环画"

# 示例3：用户要求"快速生成一张电影感的城市夜景"
# → 使用 z-image-turbo（快速+电影感），英文提示词
python scripts/z_image_gen.py text2img   "film grain, cinematic lighting, cyberpunk city at night, neon lights"   --size hd --n 1 --filename-prefix "城市夜景_赛博朋克_电影感"
```
...
```bash
# 单张图片
python scripts/qwen_image_gen.py text2img "一只可爱的橘猫趴在窗台上"   --model default --filename-prefix "可爱橘猫_窗台_温暖治愈"
