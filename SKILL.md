...
2. **提示词调整说明**（如有）："已优化提示词，增加了光线和风格描述"
3. **保存路径**：图片文件的完整路径
...
📝 **提示词优化**：
- 原始：画一张春天的海报，要有花朵
- 优化后：画面主体：春日花园；场景：花朵盛开，阳光明媚；光线：温暖的自然光；风格：清新治愈
...
- [ ] **提示词处理符合用户要求**：如果用户说"不要优化"，确认提示词未被修改；如果用户未说，确认已做风格适配
...
| 测试场景 | 用户输入 | 预期行为 | 通过标准 |
|---------|---------|---------|---------|
| **正常文生图** | "画一只橘猫趴在窗台上晒太阳" | 推荐模型 → 优化提示词 → 生成 → 保存 → 展示路径 | 图片生成成功 + sidecar 完整 + 回复含模型理由和路径 |
| **指定模型+参数** | "用 wan2.7 画 3 张小猫日常组图" | 尊重用户选择，n=3，enable_sequential=True | 生成 3 张图片 + 文件名含 seq01/02/03 + sidecar 记录参数 |
| **图生图** | "把这张图片变成水彩画风格 /tmp/photo.jpg" | 检测图片存在 → 推荐 wan/qwen → 生成 | 图生图成功 + sidecar 记录 input_image 路径 |
