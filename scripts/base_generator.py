...
class BaseImageGenerator:
    ...
    def _download_image(self, url: str, save_path: Path, max_retries: int = None) -> bool:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # 验证是否为有效图片（简单检查 content-type 和文件大小）
    ...
    def _save_sidecar(
        ...
        Args:
            ...
            extra_params: 额外参数（如 watermark, enable_sequential 等）
    ...
    def _process_response(
        ...
        Args:
            response: API 返回的响应对象（支持属性访问和字典访问）
        ...
        try:
            ...
            # === 检查响应状态（兼容属性和字典两种访问方式） ===
            status_code = (
                getattr(response, 'status_code', None) 
                if hasattr(response, 'status_code')
                else response.get('status_code')
            )
            
            # HTTPStatus.OK 的 value 是 200
            status_value = status_code.value if hasattr(status_code, 'value') else status_code
            if status_value != 200:
                error_code = (
                    getattr(response, 'code', 'Unknown')
                    if hasattr(response, 'code')
                    else response.get('code', 'Unknown')
                )
                error_msg = (
